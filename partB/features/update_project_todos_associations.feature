Feature: Modify Project-Todo Associations
  As a user
  I want to modify existing associations between projects and todos
  So that I can update how tasks are organized within projects
  ...
  Scenario Outline: Successfully update a project's todo association
    ...
    Given a todo item with title "<old_todo_title>" exists
    Given a todo item with title "<old_todo_title>" exists
    When I add the todo to the project
    Given a todo item with title "<new_todo_title>" exists
  ...
  Scenario Outline: Successfully add multiple new todos to a project while removing old ones
    Given a project with title "{project_title}" exists
    Given a todo item with title "<old_todo1>" exists

    When I add the todo to the project
    Given a todo item with title "<old_todo2>" exists
    When I add the todo to the project

    Given a todo item with title "<new_todo1>" exists
    Given a todo item with title "<new_todo2>" exists
  ...
  Scenario Outline: Fail to update project with a non-existing todo
    Given a project with title "{project_title}" exists
    Given a todo item with title "<existing_todo_title>" exists
